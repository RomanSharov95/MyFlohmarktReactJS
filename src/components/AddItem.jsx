...
export default function AddItem({ type, onCancel, onSave }) {
  // type is 'event' (Flohmarkt) or 'item' (Inserat)
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                <input
                  type="date"
                  className="flex-1 border border-slate-200 rounded-xl py-3 px-4 text-sm outline-none"
                  onChange={(e) => handleChange('date', e.target.value)}
                />
                <span className="text-sm">von</span>
                <input
                  type="time"
                  className="w-24 border border-slate-200 rounded-xl py-3 px-4 text-sm outline-none"
            ...
            <select
              className="w-full bg-white border border-slate-200 rounded-xl py-3 px-4 text-sm outline-none appearance-none"
            ...
            <select
              className="w-full bg-white border border-slate-200 rounded-xl py-3 px-4 text-sm outline-none"
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl text-xs font-bold border transition-all ${
                      formData.features.includes(feature)
                        ? 'bg-[#52A7E0] text-white border-[#52A7E0]'
                        : 'bg-[#D1E1F0] text-[#52A7E0] border-transparent'
                    }`}
                  >
                    + {feature}
                  </button>
...
function InputField({ label, name, placeholder, type = 'text', fullWidth = true, onChange }) {
  return (
    <div className={fullWidth ? 'w-full' : 'flex-1'}>
      {label && (
        <label className="block text-xs font-bold text-slate-900 mb-1 ml-1">{label}</label>
      )}
      <input
        type={type}
