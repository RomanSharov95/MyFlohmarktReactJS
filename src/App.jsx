...
function App() {
  ...
  return (
    <div className="App bg-[#F8FAFC] min-h-screen">
      ...
      {/* КОНТЕНТ */}
      <main>
        {currentPage === 'home' ? <Home /> : <Inserate />}
      </main>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-slate-100 z-[1000]">
        <div className="max-w-[1080px] mx-auto px-6 md:px-12 py-4 flex justify-between items-center">
          ...
          <div 
            ...
            className={`flex flex-col items-center gap-1 cursor-pointer transition-all ${currentPage === 'home' ? 'text-[#52A7E0]' : 'opacity-40'}`}
          >
            <div className={`${currentPage === 'home' ? 'bg-[#52A7E0] text-white' : ''} px-4 py-1.5 rounded-2xl`}>
          ...
          {/* Inserate */}
          <div 
            onClick={() => setCurrentPage('inserate')}
            className={`flex flex-col items-center gap-1 cursor-pointer transition-all ${currentPage === 'inserate' ? 'text-[#52A7E0]' : 'opacity-40'}`}
          >
            <div className={`${currentPage === 'inserate' ? 'bg-[#52A7E0] text-white' : ''} px-4 py-1.5 rounded-2xl`}>
